...
export default function ProjectCard({ slug, title, description, tech, github, live, image }) {
    const navigate = useNavigate();

    const handleCardClick = (e) => {
        // Prevent navigating to detail page if the user clicked on an external link or button
    ...
    return (
        <div
            ...
            className="group relative bg-slate-900/50 rounded-3xl overflow-hidden border border-white/5 hover:border-indigo-500/30 transition-all duration-300 flex flex-col h-full hover:shadow-2xl hover:shadow-indigo-500/10 hover:-translate-y-2 cursor-pointer"
        >
            {/* Image Area */}
            <div className="h-56 bg-slate-800/50 relative overflow-hidden">
                ...
                <div className="absolute inset-0 flex items-center justify-center z-20 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <Link
                        ...
                        className="px-5 py-2.5 bg-black/60 backdrop-blur-md text-white text-sm font-semibold rounded-full border border-white/10 flex items-center gap-2 transform translate-y-2 group-hover:translate-y-0 transition-transform duration-300 hover:bg-black/80 active:scale-95"
                    >
                        View Details
            ...
            <div className="p-8 flex flex-col flex-grow">
                ...
                <p className="text-slate-400 mb-6 flex-grow text-sm leading-relaxed font-normal line-clamp-3">
                    {description}
                </p>
